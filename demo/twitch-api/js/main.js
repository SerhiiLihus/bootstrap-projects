...
$(document).ready(function() {
  ...
  $(function() {
    ...
    for (let i = 0, count = 0; i < channels_length; i++) {
      $.getJSON(
        ...
        function(response) {
          ...
          getStream(channels[i], function(channel, stream) {
            if (stream) {
              $("#list-of-channels")
                .find("#" + channel.toLowerCase() + " .channel-status")
                .addClass("channel-online");
              $("#list-of-channels")
                .find("#" + channel.toLowerCase() + " .current-stream")
                .text(stream.channel.status);
              $("#list-of-channels")
                .find("#" + channel.toLowerCase() + " a")
  ...
  function formList(response) {
    ...
    display +=
      "<div id='" +
      response.display_name.toLowerCase() +
      "' class='channel-item'>";
